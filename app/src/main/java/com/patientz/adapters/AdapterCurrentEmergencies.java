...
public class AdapterCurrentEmergencies extends RecyclerView
        .Adapter<AdapterCurrentEmergencies
        .DataObjectHolder> {
    ...
    public void setOnItemClickListener(MyClickListener myClickListener) {
        AdapterCurrentEmergencies.myClickListener = myClickListener;
    }

    public AdapterCurrentEmergencies(Context applicationContext, ArrayList<PatientUserVO> myDataset) {
        mDataset = myDataset;
    ...
    public void onBindViewHolder(DataObjectHolder holder, int position) {
        ...
        if(!TextUtils.isEmpty(mPatientUserVO.getRelationship()))
        {
            if (mPatientUserVO.getRelationship().equalsIgnoreCase("Self")) {
                holder.tvUserName.setText(applicationContext.getString(R.string.self));
            } else {
    ...
    private void setUserPic(PatientUserVO mPatientUserVO, ImageView ivPic) {
        ...
        String url = WebServiceUrls.serverUrl + WebServiceUrls.getPatientAttachment + mPatientUserVO.getPicId()+"&patientId="+mPatientUserVO.getPicId()+"&moduleType="+ Constant.IMAGE_MODULE_TYPE_PROFILE_PIC;
        Log.d(TAG, "Url=" + url);
        Picasso
