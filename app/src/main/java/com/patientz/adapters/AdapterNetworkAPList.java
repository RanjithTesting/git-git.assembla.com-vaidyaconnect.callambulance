...
public class AdapterNetworkAPList extends RecyclerView
        .Adapter<AdapterNetworkAPList
        .DataObjectHolder> {
    ...
    public static class DataObjectHolder extends RecyclerView.ViewHolder {
        TextView tvHospitalName, tvBranchName, tvDistanceInKms, tvTimeToReach;
    ...
    public AdapterNetworkAPList(OrgBranchesActivity applicationContext, List<OrgBranchVO> myDataset, CountDownTimer countDownTimer) {
        mDataset = myDataset;
    ...
    public void onBindViewHolder(DataObjectHolder holder, int position) {
        final OrgBranchVO orgBranchVO = mDataset.get(position);
        holder.tvHospitalName.setText(!TextUtils.isEmpty(orgBranchVO.getDisplayName()) ? (orgBranchVO.getDisplayName()) : "");
        //setPhoneNumber(hospitalVO, holder.tvHospitalPno);
        setAddress(orgBranchVO, holder);

        //setDistanceTimeToReach(orgBranchVO, holder.tvDistanceInKms);

       /* if(orgBranchVO.getAddress()!=null)
        {
            holder.tvBranchName.setText(!TextUtils.isEmpty(orgBranchVO.getAddress().getStreetAddress())?orgBranchVO.getAddress().getStreetAddress():"");
        }else
        {
        ...
        if (AppUtil.isOnline(applicationContext)) {
            mRequestQueue.add(createTimeCalRequestToDestination(orgBranchVO, holder.tvTimeToReach, holder.tvDistanceInKms, currentLocationLat, currentLocationLon));
        } else {
            holder.tvDistanceInKms.setText(CommonUtils.getTwoDigitDecimalValue(orgBranchVO.getDistanceInKms()) + " Km");
        }
        holder.parentLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                setProviderToCall(orgBranchVO);
                SharedPreferences mSharedPreferences = PreferenceManager.getDefaultSharedPreferences(applicationContext);
                mSharedPreferences.edit().putLong("bId", orgBranchVO.getOrgBranchId()).commit();
                Log.e("bid-->3", "" + orgBranchVO.getOrgBranchId());
                Log.e("bid-->3", "" + orgBranchVO.getDisplayName());
            }
        });
        ArrayList<OrgBranchSpecialityVO> specialities = orgBranchVO.getSpecialities();
        ArrayList<OrgBranchFacilityVO> facilities = orgBranchVO.getFacilities();
        ...
        holder.llMore.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(final View v) {
                ...
                text_fecilities.setText(applicationContext.getString(R.string.facilities));
                TextView text_info = (TextView) dialog.findViewById(R.id.text_info);
                text_info.setText(applicationContext.getString(R.string.specalities));
...
/*				/*		AppUtil.showToast(getApplicationContext(),
                ...
                if (error instanceof TimeoutError) {
                    Log.d(TAG + "_OnErrorResponse", "TimeoutError");

                } else if (error instanceof NoConnectionError
                        || error instanceof NetworkError) {
                    Log.d(TAG + "_OnErrorResponse",
                            "NoConnectionError");

                } else if (error instanceof ParseError) {

                    Log.d(TAG + "_OnErrorResponse", "ParseError");
                }
                // finish();*/
            }
    ...
    private void setAddress(OrgBranchVO orgBranchVO, DataObjectHolder dataObjectHolder) {
    ...
    private void setProviderToCall(OrgBranchVO hospitalVO) {
        ...
        mEditor.putString("emergencyNumber", hospitalVO.getAmbulanceProviderPhoneNo());
        mEditor.commit();*/

        Intent intent = new Intent(applicationContext, ActivityEmergencyStepsListener.class);
